    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
      <div className={`${styles.flexCenter} md:flex-row flex-col mb-8 w-full`}>
        <div className="flex-1 flex flex-col justify-start mr-10">
          <img src={logo} alt="" className="w-[260px] h-[72px] object-contain" />
        ...
        <div className={`flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10`}>
          {footerLinks.map((footer)=>(
            <div key={footer.key} className="flex flex-col sm:my-0 my-4 min-w-[150px]">
      ...
      <div className={`w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3e45]`}>
        ...
        <div className="flex flex-row md:mt-0 mt-6">
          {socialMedia.map((sc,index)=>(
            <img src={sc.icon} key={sc.id} className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== sc.length - 1 ? 'mr-6' : 'mr-0'}`}/>
          ))}
        </div>
      </div>
    </section>
